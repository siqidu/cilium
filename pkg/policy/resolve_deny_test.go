...
func (ds *PolicyTestSuite) TestMapStateWithIngressDeny(c *C) {
	...
	c.Assert(policy.policyMapChanges.adds, HasLen, 0)
	c.Assert(policy.policyMapChanges.deletes, HasLen, 0)
	c.Assert(policy.policyDenyMapChanges.adds, HasLen, 3)
	c.Assert(policy.policyDenyMapChanges.deletes, HasLen, 0)
	...
	c.Assert(policy.policyMapChanges.adds, HasLen, 0)
	c.Assert(policy.policyMapChanges.deletes, HasLen, 0)
	c.Assert(policy.policyDenyMapChanges.adds, HasLen, 2)
	c.Assert(policy.policyDenyMapChanges.deletes, HasLen, 1)
	...
	allowEgressMapStateEntry := NewMapStateEntry(labels.LabelArrayList{ruleLabelDenyAnyEgress}, false)
	...
	c.Assert(policy.policyDenyMapChanges.adds, IsNil)
	c.Assert(policy.policyDenyMapChanges.deletes, IsNil)

	c.Assert(allowAdds, IsNil)
	c.Assert(allowDeletes, IsNil)
	c.Assert(denyAdds, checker.Equals, MapState{
		{Identity: 192, DestPort: 80, Nexthdr: 6}: rule1MapStateEntry,
		{Identity: 194, DestPort: 80, Nexthdr: 6}: rule1MapStateEntry,
	})
	c.Assert(denyDeletes, checker.Equals, MapState{
		{Identity: 193, DestPort: 80, Nexthdr: 6}: rule1MapStateEntry,
	})
}
